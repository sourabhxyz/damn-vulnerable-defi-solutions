...
describe('[Challenge] Free Rider', function () {
  ...
  before(async function () {
    ...
    this.uniswapPair = UniswapPairFactory.attach(
    ...
    this.nft = DamnValuableNFTFactory.attach(await this.marketplace.token());
  ...
  it('Exploit', async function () {
    ...
    attackFreeRiderNFTMarketplace = await (
      await ethers.getContractFactory('AttackFreeRiderNFTMarketplace', attacker)
    ).deploy(
      this.weth.address,
    ...
    await attackFreeRiderNFTMarketplace.connect(attacker).attack();
    console.log(
      'Attacker ends with: ',
      ethers.utils.formatEther(
        await ethers.provider.getBalance(attacker.address)
